...
public abstract class SwaggerUtils {
    ...
    public static Docket initDocket(String groupName, Predicate<RequestHandler> selector,
                                    YeyueSwaggerProperties properties, OpenApiExtensionResolver resolver, ErrorCode[] codes) {
        // 创建 Docket 对象
        return new Docket(DocumentationType.OAS_30)
                // 用来创建该 API 的基本信息，展示在文档的页面中（自定义展示的信息）
                .apiInfo(apiInfo(properties)).globalResponses(HttpMethod.GET, globalResponses(codes))
                ...
                .directModelSubstitute(LocalDateTime.class, String.class).useDefaultResponseMessages(false)
                // default配置默认读取所有接口
                .select().apis(selector).paths(PathSelectors.any()).build().groupName(groupName)
                .globalRequestParameters(globalRequestParameters()).extensions(resolver.buildSettingExtensions());
    }
    ...
    private static List<RequestParameter> globalRequestParameters() {
        RequestParameterBuilder tenantParameter = new RequestParameterBuilder()
                .name(ServletConstants.AUTHORIZATION_HEADER).description("鉴权信息").required(false).in(ParameterType.HEADER);
